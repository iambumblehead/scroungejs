...
var Message = module.exports = {
   ...
   */
  compressingFile : function (fileObj, tree, progress, opts) {
    var tpl = "[...] ugly: (:treename, :treetype, :progress) :filename";

//    console.log(fileObj.filename, opts.outputPath);
    return tpl
      .replace(':treename', tree)
      .replace(':treetype', fileObj.type)
      .replace(':progress', progress)
      .replace(':filename', fileObj.filename);//pathpublic.get(fileObj.filename, opts.inputPath));
  },
  ...
  pathInvalid : function (path) {
    return new Error("[!!!] path is invalid: " + path);
  },
