...
var FileUtilityJS = module.exports = {
  ...
  rmExports : function (str) {
    var exportsRe = /= module.exports =|= \(\(typeof module === 'object'\) \? module : \{\}\)\.exports =/gi, 
        newStr;

    newStr = str.replace(exportsRe, function (match) { return '='; });
    return newStr;
  },
  ...
  preProcess : function (str, opts, fn) {
    ...
    if (opts.isRemoveConsole) {
      cmprStr = that.rmConsole(cmprStr);
    }
    if (opts.isRemoveRequires) {
      cmprStr = that.rmExports(cmprStr);
    }
