...
var Graph = module.exports = (function () {
  var graph = {
    ...
    // nodes here extracted here as a tree may 
    // have other nodes dependent upon them... (they are leaves)
    extractAsTree : function (nodeName) { 
      var tree = this.vertexMap[nodeName], treeContent;
    ...
    extractAsTreeArr : function (nodeNameArr) { 
      var that = this, treeArr = [];

      nodeNameArr.map(function (nodename) {
        var tree = that.extractAsTree(nodename);
        if (tree) { 
          treeArr.push(tree);
        }
      });
