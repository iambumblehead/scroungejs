var fs = require('fs'), // read/write files
    path = require('path'),
    BMBLib = require('./BMBLib.js'), // css compression    
...
// acquires objects with file information
// necesssary for scrounge management
var InfoFile = module.exports = (function() {
  var infoFile = {
    filename : '', // in tree w/fullpath
    treename : '', // in tree
    dependencryArr : [],
    type : '', // css|js
    ...
    //writeOutput : function (outputDir, isTimestamped, text, funchandle) {
    ...
    inspectFileText : function (text) {
      ...
      for (x = traceArr.length; x--;) {
        results.push("[WWW] trace: " + filename + ", " + traceArr[x]);      
      }
    },
  ...
  return {
    getFromFile : function (fd, filename) {
      ...
      if (traces.length) Message.storeMessage(['[MMM]: trace @ ' + treename, traces]);
