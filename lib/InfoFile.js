...
var InfoFile = module.exports = (function() {
  ...
  return {
    getNew : function (spec) {
      ...
      that.treename = spec.treename;
      that.type = spec.type;
      that.dependencyArr = spec.dependencyArr || [];
      that.timestamp = spec.timestamp || new Date();
      that.authors = spec.authors;
      return that;
    },

    getFromFileFileName : function (fn, filename) {
    ...
    getFromFile : function (fd, filename) {
      ...
      return this.getNew({
        ...
        type : filename.match(/.js$/) ? 'js' : 'css',
        dependencyArr : parser.getDependencies(fd, filename),
        timestamp : parser.getTimestamp(fd, filename),
        authors : parser.getAuthors(fd)
      });
    }
  };
