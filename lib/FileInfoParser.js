...
var FileInfoParser = module.exports = {
  ...
  getDependencies : function (file) {
    //var requiresRe = /Requires:((\/\/)? ?[\w.-]*\.[cj]ss?([,\r\b ]?)*(\n\/\/ )?)*/,
    var requiresRe = /Requires: ?[,\r\n ]?((\/\/)? ?[\w.-]*\.[cj]ss?([,\r\b ]?)*(\n\/\/ )?)*/,
        filenameRe = /[\w.-]*\.[cj]ss?/g,
        match = file.match(requiresRe),
        dependencyArr = [];

    // match the requires line followed by next // comment line
    // until no comment line or comment line is empty

    //requiresRe = /Requires: ((\/\/)? ?([\w\.]|\.(?![cj]ss?))*\.[cj]ss?([,\r\b ]?)*(\n\/\/ )?)*/;
