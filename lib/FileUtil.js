...
var FileUtil = module.exports = {
  ...
  // 
  // getPublicPath('/home/scr/go/b.js', './scr')   // './scr/go/b.js';
  // getPublicPath('/home/scr/go/b.js', '/fe/scr') // '/fe/scr/go/b.js';
  // getPublicPath('/home/scr/go/b.js', 'http://www.google.com/fe/scr') 
  // 'http:www.google.com/fe/scr/go/b.js';
  getPublicPath : function(sysPath, pubPath) {
    var sysPathArr, 
        pubPathArr,
        newPath = '', x, pos, sep = path.sep;

    if ((typeof sysPath === 'string') &&
        (typeof pubPath === 'string')) {
      sysPathArr = sysPath.split(sep);
      pubPathArr = pubPath.split(sep);      

      for (x = pubPathArr.length; x--;) {
        if (pubPathArr[x]) {
          newPath = pubPathArr[x];
          break;
        }
      }

      if (newPath) {
        pos = sysPathArr.indexOf(newPath);
        if (pos !== -1) {
          newPath = pubPathArr.slice(0, x)
            .concat(sysPathArr.slice(pos))
            .join(sep);
        }
      }
    }

    return newPath || sysPath;
  },
