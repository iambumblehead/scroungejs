...
var FileUtil = module.exports = {
  ...
  createPath : function (directory, fn) {
    var path = FileUtil.getFullPath(directory);
  
    fs.stat(path, function (err, stat) { 
      if (stat && stat.isDirectory()) {
        fn(null, directory);
      } else {
        nodefs.mkdir(path, 0755, true, function (err, res) {
          if (err) return fn(err);
          fn(err, res);
        });
      }
  ...
  rmSimilarFiles : function (filename, outputdir, fn) {
    ...
    var outDir = FileUtil.getFullPath(outputdir);
    fs.stat(outDir, function(err, stat) {      
      if (err || !stat) return fn(Message.pathOutputInvalid(outDir));
      if (stat.isDirectory()) {
        fs.readdir(outDir, function(err, pathnameArr) {
          if (err) return fn(err);
          for (x = pathnameArr.length; x--;) {
            if (stripFn(pathnameArr[x]) === matchname) {
              return fs.unlink(path.join(outDir, pathnameArr[x]), fn);
            }
          }
  ...
  isPassingFilename : function (filename, opts) {
    ...
    if (!fileextn.match(/(js|css|less)$/)) return false;
