...
var FileInfoNode = module.exports = (function() {
  var fileInfoNode = {
    ...
    isConcatenated : function (opts) {
      // this.type is .less, .js, .css or .mustache
      // types should be changed in future to:
      // - `template`
      // - `stylesheet` 
      // - `javascript`
      //
      // for now we add a primitive check for the type
      var type = this.type;
      
      if (type.match(/mustache|html|jsp/gi)) {
        return false;
      } else {
    ...
    getFileUtility : function () {
      ...
      if (type.match(/\.js/)) {
        ...
        return UtilityCSS;
      } else if (type.match(/\.mustache|\.html/)) {
        return UtilityHTML;
      }
      throw new Error('file extension not found: ' + type);
    },
    ...
    getCmprNameStr : function (opts) {
      ...
      return fileNameCmpr.replace(/less$/, 'css');
    },
    ...
    getPublicPathStr : function (opts) {
      var that = this, 
          fileName = that.getCmprNameStr(opts), 
          filePath;

      if (opts.outputPath) {
        filePath = path.join(that.getFullPath(opts.outputPath), fileName);                
    ...
    getContentStr : function (opts, fn) {
      var thisFilePath = FileUtil.getFullPath(this.filename);
      
      if (!thisFilePath) return fn(Message.dependencyNotFound(this));
      fs.readFile(thisFilePath, 'utf-8', fn);
    },
    ...
    getContentProcessedStr : function (opts, fn) {
      ...
      that.getContentStr(opts, function (err, res) {
        ...
        Utility.preProcess(res, opts, function (err, res) {
          if (err) return fn(err);
    ...
    writeOutput : function (outputDir, str, opts, fn) {
      var filename = this.getCmprNameStr(opts),
          outputPath = FileUtil.getFullPath(path.join(outputDir, filename));

      if (outputDir.match(/(\.js|\.css)$/)) {
        console.log(Message.savingFile(outputDir));
        outputPath = FileUtil.getFullPath(outputDir);
        ...
        FileUtil.createPath(outputDir, function (err, res) {
          ...
          FileUtil.rmSimilarFiles(filename, outputDir, function (err, res) {
            console.log(Message.savingFile(
              pathpublic.get(outputPath, opts.outputPath)
            ));
