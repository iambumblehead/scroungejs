  ...
  return {
    ...
    getFromFileArr : function (filenameArr, fn) {
      var fileObjArr = [], that = this,
          ...
          fileOpenFn = that.getFileOpenPrinterFn(length);

      (function openNext(x, filename) {
//        fileOpenFn.count(filename, x);

//        if (x - 1 === 0) fileOpenFn.doComplete();
//        if (x - 1 === 0) throw 'done';
//        console.log('x', x);
//        if (!x--) return null;
        //if (!x--) return (throw 'done');
        if (!x--) return fn(null, fileObjArr);
        if (!(filename = filenameArr[x])) return openNext(x);

        fileOpenFn.count(filename, length - x);
        
        // open file
        //console.log(Message.openFile(filename));
//        fileOpenFn.count(filename);
    ...
    getFileOpenPrinterFn : function (length) {
      var barTpl = Message.openFileTpl();
      var bar = ProgressBar.getNewCount(barTpl, {
        width : 30,
        length : length
      });
