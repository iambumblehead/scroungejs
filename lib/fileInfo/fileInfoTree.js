...
var FileInfoTree = module.exports = (function () {
  ...
  var fileInfoTree = {
    ...
    getProgressJoinPrinterFn : function (l, opts) {
      ...
      //var bar = ProgressBar.getNew('[...] join: (' + treename + ': ' +  this.fileInfoObj.filename + ') [:bar] :percent :etas', {
      ...
      //console.log(Message.joiningTreeFiles(this));
      
      return function() {
        bar.tick();
//        console.log('complete', bar.complete);
//        console.log('complete', bar.complete);
//        console.log('complete', bar.complete);
//        console.log('complete', bar.complete);
        if (bar.complete) {
    ...
    writeInfoTree : function (outputDir, opts, fn) {
      ...
      (function copyNextInfoObj(x, infoObj, progress) {
        ...
        infoObj.getContentProcessedStr(opts, function (err, str) {
          ...
          finStr += str + (isLines ? "\n" : '');
          //progressPrint(x, infoObj);
          //progressPrint(length / x + 1, infoObj);
...
//          console.log(length, length - x);
//          console.log(length, length - x);
//          progressPrint(length - x + 2, infoObj);
          copyNextInfoObj(x);
        });
      }(length));    
    },
