  ...
  var fileInfoTree = {
    ...
    getAuthorsArr : function () {
      ...
      for (x = fileObjArr.length; x--;) {
        fileObjArr[x].map(function (fileObj) {
    ...
    reportMissingDependencyArr : function (missingDependencyArr) {
      ...
      for (x = fileObjArr.length; x--;) {
        // if fileObj is not one of the missing dependencies...
        if (missingDependencyArr.indexOf(fileObjArr[x].treename) == -1) {
          // loop through dependencies
          fileObjArr[x].dependencyArr.map(function (dependency) {
            if (missingDependencyArr.indexOf(dependency) !== -1) {
              console.log(Message.missingDependency(filename, fileObjArr[x].treename));             
            }
          });
        }
      }
