...
var FilterTree = module.exports = (function () {
  ...
  var filter = {
    ...
    // trees must be added in the order they are given
    // so that they may be populated in the same order
    addTrees : function (nameArr, type) {
      var trees = this.trees, x, l;

      trees[type] = trees[type] || [];
      for (x = 0, l = nameArr.length; x < l; x++) {
    ...
    getAllFilterTreeNameStrArr : function () {
      var treeFilters = this.trees,
          treeNameStrArr = [];

      for (var o in treeFilters) {
        if (Array.isArray(treeFilters[o])) {
          treeFilters[o].map(function (treefilterStr) {
            if (treeNameStrArr.indexOf(treefilterStr) === -1) {
              treeNameStrArr.push(treefilterStr);
            }
          });
        }
      }
      return treeNameStrArr;
    },
